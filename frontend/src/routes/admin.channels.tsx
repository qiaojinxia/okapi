    ...
    <Card>
      ...
      <CardContent className="flex flex-col gap-3">
        ...
        <div className="flex flex-col gap-1.5 border-t border-border pt-3">
          ...
          {(channel.keys ?? []).map((k) => (
            <KeyParamRow key={k.id} channelId={id} row={k} onDone={onDone} onMsg={setMsg} />
          ))}
        </div>
...
/// 请求直接 404 且不易排查。手输框保留为兜底（上游有而本站还没配定价的模型）。
///
...
function ModelPicker({
  ...
  if (models.isError) {
    return <p className="text-xs text-destructive">{describeError(models.error)}</p>
  }
  return (
    <div className="flex flex-col gap-2">
      <Label>{t('admin:pickModels')}</Label>
      <p className="text-xs text-muted-foreground">{t('admin:pickModelsHint')}</p>
