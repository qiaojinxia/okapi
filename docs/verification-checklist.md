...
12. **软删留下的外键死引用未逐一排查（09-08 第十九轮，已修一处）**：用户 / 渠道 / 令牌都是软删（只置 `deleted_at`），可它们身上指向配置表的外键列并不会被清。`delete_role` 就栽在这上面——占用检查带 `deleted_at IS NULL`，墓碑上的 `admin_role_id` 既不算占用、又拦得住硬删，撞出 500 且角色永远删不掉（本轮已修：删角色前在同一事务里把墓碑引用置 NULL，定案见 IMPLEMENTATION §删除语义）。同类形状还有几处没查：`users.group_code` / `channels.pool_code` 之类"配置类硬删 + 占用检查"的组合，只要占用检查滤掉了软删行而外键没有 `ON DELETE`，就是同一个坑。下一轮按 `0001_init.sql` 里的 `REFERENCES` 列表逐个对一遍占用检查的谓词。
...
| L1 + L2 | 审计前 **529 / 529**（108 个二进制）→ 第一批补完 **533 / 533** → 机械对表再补一批后 **539 / 539**（109 个二进制）。三跑都是 `env -i` 只给 CI 那三个连接串 |
