...
#### `anthropic_to_openai` 收尾：9 条真缺口全部补上

补 `request_maps_url_images_sampling_schema_and_multipart_text`、`response_finish_reason_table`、
`usage_clamps_cache_counts_that_exceed_prompt`（连同上一节的两条）。对**修复后的代码**逐条变异：
6 条原 SURVIVED + 3 条可疑 CAUGHT(全量) 全部由新用例精确抓住；那 3 条在快筛里专属套件原本抓不住，
所以此前记下的"全量抓住"**又全是假的**（上一轮 2/2，这一轮 3/3）。清单写错而未测的 base64 那条，
实跑确认一直有覆盖（原有用例断言了 data URI）。

#### 抖动用例：查清并修掉三处，一处未坐实

它们已连续两轮把真缺口伪装成 CAUGHT(全量)（5 次），也会让 CI 偶发红。逐个查根因，只认复现过的：

| 用例 | 根因 | 证据 | 处理 |
| --- | --- | --- | --- |
| `gateway_invalid_key_rate` 两条 | 计数 `crl:invalid_api_key:{ip}` 在共享 Redis、60s 固定窗；测试 IP 一个写死 `198.51.100.9`、一个只有 250 个取值 | 旧版 60s 内连跑：第 1 次绿、第 2~4 次红；新版连跑 5 次全绿 | 每次取唯一的 IPv6 文档地址（`2001:db8::/32`，64 位随机后缀） |
| `console_stats::portal_charts_expose_…` | `drain` 一次就断言。outbox 是全局队列，并行用例持锁时本用例的行还在排队 | 辅助函数 `poll_until` 的注释早已写明此坑，其余同类写法都已迁移，唯独这处漏了 | 改为 `poll_until` |
| `console_ops::settings_get_and_leaderboard` | ① 只在轮询前 `drain` 一次，轮询里不再 drain；② 写死 77 美元挤 7 天 Top 100，而这条用例自己每跑一次就多一个 77 美元用户 | 开发库实测：7 天内 ≥77 美元的用户 **122 个**，已超过 Top 100，并列名次排序任意——随时间越跑越红 | 先读当前榜首金额、结算"榜首 + 1 美元"；drain 挪进轮询循环 |
| `gateway_group_rate` 三条 | **未坐实** | 分桶是日历对齐的 `now / 60`，跨整分钟会清零——但在第 58~59 秒掐点起跑 3 次均未复现，也解释不了"小时窗那条同时红"。Redis 淘汰键假设被数据推翻（`noeviction`、`evicted_keys = 0`） | 暂列入 sweep 的跳过名单，不当作已修 |

`gateway_group_rate` 查的过程中看到一处值得单独记的行为：`fixed_window_ok` 在 Redis 出错时**放行**，
且只打 debug 日志——Redis 抖一下，所有固定窗限流会同时、静默地失效。它能解释"三条同时红"，
但那次运行没有留下日志，**没有证据**，只作为待查线索记下。`console_manage::channel_batch_and_user_actions`
所在文件正被并行会话修改，本轮不动。

### 2026-09-24 第三十四轮：逐接口端到端探针——166 个接口，每个都问一遍"返回的内容有没有人核对"

前几轮按**域**做规则级变异，测的是内部逻辑被测得怎么样；目标要的是**每一个接口**的端到端准确性，
而此前拿不出一张逐接口的表。本轮换一个能直接回答它的判据。

#### 判据：逐个破坏一个接口的成功响应

- 环境变量 `OKAPI_E2E_PROBE="METHOD /route/{template}"` 选中**一个**接口，由一个临时挂上的 `route_layer`
  中间件把它的 **2xx 响应体**换成 `{"__e2e_probe":true}`；状态码、鉴权、错误分支、非 2xx 一概原样放行。
  于是匿名探测、越权扫描这类横切用例不会被误触发，测到的纯粹是**内容**。
- 中间件在路由匹配之后运行，拿得到 `MatchedPath`（路由模板）。补丁只在 worktree 上打，**不进产品代码**。
- 运行期开关：**只编译一次**，每个接口只换环境变量。
- 定向集合 = 源码里匹配该路由模式（`{param}` 当通配）的测试文件；没抓住时做"前缀复核"
  （提到父路径的文件，兜住"先查列表再拼详情地址"）。
- 先在两个已知答案的接口上校验判据本身：`GET /admin/margin-breaker` 应 CAUGHT、`GET /v1/models`
  应 SURVIVED（已提交的树上零内容覆盖）——两者都对上才开跑。
...
| --- | --- |
| 原有用例已端到端核对内容 | 114 |
| 改判为已核对（真实验证者曾被误列为抖动用例，见下） | 3 |
| **本轮补用例后核对** | **20** |
| 探针不适用：3xx 跳转（OAuth 起跳 / 回调）、101 升级（`/v1/realtime`） | 3 |
| 未核对：`/v1/models`（并行会话未提交的用例已覆盖同一变异） | 1 |
| 未核对：只回 `{"ok":true}` 的写接口——内容无意义，准确性在副作用，需另立口径 | 25 |

本轮补上的 20 个里，有几处不只是"回执没人看"：

- **`translations_bills_per_call` 名实不符**：名为"按次计费"，实际只断言了 200。按 transcriptions
  同一契约补上返回体、金额与快照。
- **自定义透传从没走过 POST**：路由是 `any(...)`，原用例全走 GET。补 POST 用例：方法、查询串、
  Content-Type、请求体逐字节原样到上游，且**客户端的 Okapi token 不随请求转发给上游**。
- **改凭据的副作用没人验**：补的用例核对新凭据加密落库、解开等于新值、key 状态机复位，
  以及网关下一次打上游**确实换用了新凭据**。副作用变异三条（存明文、不复位、不清缓存）全部抓住——
  其中"不清缓存"第一版没抓住：用例改凭据前从没走过网关，候选缓存是冷的。候选缓存里存的是
  **已解封的凭据本身**（TTL 5s），改为先请求一次灌热缓存后，精确红在上游凭据那一行。
- **管理端订阅的查询与取消在用例里从没被调用过**：补的用例要求管理端看到的订阅与用户自己在门户
  看到的逐字相同（两边同一函数出数，视图里没有随时间变化的字段），取消回执带被取消的那条。
- 删 key（管理端 / 自助）核对回执之外，再核"立刻失效"——该文件与 `console_users` 一样串行执行，
  理由同第三十二轮：并行用例的全局缓存清空会掩盖"没按 key 删缓存"的回归。

#### 过程中的方法学事故

1. **全量复核被抖动用例系统性污染。** 第一段 64 个接口里，9 个"全量才抓住"的抓手全是
   `portal_charts_expose_…` / `settings_get_and_leaderboard`——worktree 里跑的是它们修复前的版本
   （第三十三轮已查清并修）。改为：判定时把已知抖动用例从抓手名单里剔除、另列旁注。
2. **旁注救回了三个接口。** `GET /v1/videos/{task_id}`、`…/content`、`GET /admin/stats/realtime`
   在剔除后成了 SURVIVED，但旁注显示唯一有反应的正是 `videos_create_poll_download_bills_per_seconds`、
   `realtime_kpi_counts_settlements`——**它们就是这些接口真正的验证者**，只是还会在无关场合偶发红。
   若当初把抖动用例一剔了之，这三个接口会被误判为缺口。
3. **全量复核改为前缀复核。** 两个 worker 并行时单次全量要两个多小时；而第一段 22 次全量复核，
   真正多抓到的次数为 0。改判口径时已在结论里注明。
4. **脚本三次静默出错**：传相对路径、`cd` 进 worktree 后找不到清单，循环一个接口都不跑就宣布完成；
   清理函数 `git checkout -- .` 把放进 worktree 的测试修复一并抹掉；杀进程时父进程死后循环又派生新子进程。
   均已修（绝对路径 + 清单缺失即报错、清理只还原探针动过的三个文件、按命令行模式反复清理）。
5. **两套全量并跑会互相干扰**：一次基线因 `price_group_pagination_matches_database_pages` 红而中止
   （基线闸按设计拒绝继续）；另有 4 个"全量才抓住"的抓手所在文件根本不引用该路由——按 SURVIVED 计。
6. **改写查询宏被 clippy 抓住。** 为避开 `.sqlx` 离线缓存，把新用例里的 `query!` 宏改写成运行期
   `sqlx::query*` 之后，原本藏在宏里的写法（不需要井号的 `r#"…"#`、函数体中间的 `type` 别名）
   暴露给了 clippy pedantic，提交闸 9 处报错；另有一处 `map().unwrap_or()` 应为 `map_or()`。
   clippy 在一个目标失败后不保证检查完其余目标，修完后整仓重跑确认零告警。
