...
-- KEYS[1] bal:{uid}
-- ARGV[1] target_micro —— billing_events 同池求和，事件流是权威源
-- ARGV[2] pool_field —— 'avail'（钱包，pool 0）或 'sub'（订阅池，pool 1）；缺省 avail
-- 返回 {旧值, 新值, 同池在途合计}（一律字符串，避免 Lua double 截断大额）
--
-- 在途预扣**不动**：它们各自会按结算/退款路径终结，现在抹掉会让那些请求
-- 结算时凭空多扣或少扣。所以设 池 = target - Σ同池在途，保证对账不变式
-- `池 + 同池在途 == 该池账本` 成立；扫描与写入同一脚本内完成，与并发 reserve 原子互斥。
-- 另一个池及其在途一律不碰。
--
